...
function set_datetime_text(source, format)
	...
	local gio = tonumber(os.date("%H"))
	local phut = tonumber(os.date("%M"))

	-- Tách từng chữ số của giờ
	local gio_1 = math.floor(gio / 10)
	local gio_2 = gio % 10

	-- Tách từng chữ số của phút
	local phut_1 = math.floor(phut / 10)
	local phut_2 = phut % 10

	-- Ghép lại thành chuỗi mong muốn
	local text = string.format("%d %d : %d %d", gio_1, gio_2, phut_1, phut_2)

	-- Cập nhật text source
